...
from agent_factory.agents.mixins.critic.tdqc_common import TDQCBatchMixin, TDQCMLPConfig
from agent_factory.modules.risk_predictors.tdqc import TDQCMLP, masked_bce_loss, masked_td0_loss


class TDQCMLPMixin(TDQCBatchMixin, CriticEvalMixinBase):
    CONFIG_CLASS = TDQCMLPConfig
    CONFIG_KEY = "critic"
    RESOLVE_RULES = {
        "input_dim": {
            "source": "agent_sp.encoder_override.out_dim",
            "required": False,
        },
    }
    ...
    def _build_tdqc_predictor(self):
        ...
        self.tdqc_predictor = TDQCMLP(
            input_dim=int(getattr(cfg, "input_dim", 0)),
            hidden_dims=list(getattr(cfg, "hidden_dims", [256, 256])),
        )
    ...
    def update_tdqc(self, batch: Dict[str, Any]) -> Dict[str, float]:
        ...
        logits = self.tdqc_predictor(step_features)
        ...
        if loss_type in {"bce", "mc", "monte_carlo"}:
            ...
            with torch.no_grad():
                target_logits = self.tdqc_target_predictor(step_features)
        ...
        loss.backward()
        grad_clip = float(getattr(self.cfg.critic, "grad_clip_norm", 0.0))
        if grad_clip > 0:
    ...
    def train_tdqc_loop(self, dataloader, num_steps: int, save_dir: str = ""):
        ...
        if hasattr(dataloader, "__len__") and len(dataloader) <= 0:
            raise ValueError("TDQC dataloader is empty; cannot train.")
        ...
        pbar = tqdm(range(num_steps), desc="Train TDQC MLP", leave=True)
        for step_idx in pbar:
            ...
            if save_dir and (step_idx + 1) % save_interval == 0:
                ...
                self.save(
                    os.path.join(save_dir, f"tdqc_mlp_step_{step_idx + 1}.pth"),
                    meta={"mode": "tdqc_mlp"},
                )
    ...
    def predict_success_batch(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        if valid_mask is not None:
            valid_mask = valid_mask.to(self.device, dtype=torch.float32)
        logits = self.tdqc_predictor(step_features)
        success_prob = torch.sigmoid(logits)
        return self._select_eval_timestep(success_prob, valid_mask).detach().cpu()
    ...
    def eval_risk_batch(self, batch: Dict[str, Any], only_obs: bool = True) -> Dict[str, Any]:
        ...
        valid_mask_cpu = valid_mask.detach().cpu() if valid_mask is not None else torch.ones_like(success_prob_all_cpu)
        ...
        outputs: Dict[str, Any] = {
            ...
            "risk_signal/method_id": "tdqc_mlp",
            "risk_signal/display_name": "TDQC MLP",
