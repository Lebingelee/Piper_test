...
- `HITLRunner` should treat Piper `t` teleop as an env-side intervention source. It reads `env.unwrapped.is_teleop_enabled()` before each step, clears stale action chunks during intervention, and replans after release.
- Runner-side keyboard override should use `o`; PiperEnv teleop should use `t`. Avoid assigning both systems to `t`.
...
- PiperEnv exposes `set_master_follow(bool)` / `switch_master_follow(str)` and reads default `common.master_follow` from `dual_piper_config.yaml`.
- `begin_master_follow()` / `end_master_follow()` must not run inside the high-frequency `step()` loop. Mode switches should happen only on state boundaries: `switch_passive`, `set_master_follow`, `t` teleop toggle, or close/reset paths.
...
- During `t` teleop, the master arm should be released to draggable leader mode and the follower should track fresh leader readings. While `tele_enabled=True` but leader readings are not ready, PiperEnv must hold with `get_safe_action()` rather than executing stale policy chunks.
