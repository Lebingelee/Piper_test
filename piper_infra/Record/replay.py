...
def replay_trajectory(dataset_path: str, episode_idx: int = -1, fps: int = None, ctrl_mode: str = "joint"):
    ...
    if replay_fps is not None and replay_fps != "" and float(replay_fps) > 0:
        replay_fps = float(replay_fps)
    else:
        # 尝试从数据集获取 fps，LeRobot 标准是数据集本身定义了 fps
        if hasattr(dataset, 'fps'):
            replay_fps = dataset.fps
        elif hasattr(dataset, 'meta') and hasattr(dataset.meta, 'fps'):
            replay_fps = dataset.meta.fps
        else:
            replay_fps = 30.0
