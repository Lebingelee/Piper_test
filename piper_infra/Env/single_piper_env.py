...
class PiperTeleopEnv(gym.Env):
        ...
        try:
            while self._master_running:
                ...
                with self._lock:
                    ...
                    if mja is not None: 
                        self._latest_master_joints[:6] = mja.msg
                    
                    if mep is not None: 
                        self._latest_master_pose[:6] = mep  
            ...
            print(f"\n[致命错误] 主臂读取线程崩溃: {e}\n")
            import traceback
            traceback.print_exc() # 把更详细的死因打印出来，以防万一
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, Any], float, bool, bool, Dict[str, Any]]:
        ...
        target_width = np.clip(executed_action[6] * 0.5, 0.001, 0.1)
    ...
    def reset_to_state(self, joint_angles: np.ndarray, gripper_width: float, wait_time: float = 3.0):
        ...
        self.follower_eff.move_gripper(np.clip(gripper_width * 0.5, 0.001, 0.1))
